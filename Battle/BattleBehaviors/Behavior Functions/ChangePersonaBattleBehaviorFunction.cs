...
namespace Grawly.Battle.Functions {
	...
	public class ChangePersonaBattleBehaviorFunction : BattleBehaviorFunction {
		...
		/// </summary>
		private static string descriptionText = "Changes the Persona of the specified combatant.";
		/// <summary>
		...
		private IEnumerator ExecuteAsCoroutine(Combatant source, List<Combatant> targets, BattleBehavior battleBehavior) {
			// CanvasController.Instance.SetPersonaSelectionCanvas(false);
			CombatantAnalysisCanvas.instance.Clear();

			// Play the change animation.
			yield return PersonaChangeAnimationController.instance.PlayPersonaChangeAnimation(
				sourcePlayer: source as Player, 
				oldPersona: (source as Player).ActivePersona, 
				newPersona: (CombatantAnalysisCanvas.instance.selectedCombatant as Persona));

			// yield return new WaitForSeconds(1.0f);
			// Assign a new persona to the player.
			((Player)BattleController.Instance.CurrentCombatant).AssignPlayerPersona(persona: (Persona)CombatantAnalysisCanvas.instance.selectedCombatant);
