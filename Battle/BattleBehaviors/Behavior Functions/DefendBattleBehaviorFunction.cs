...
using UnityEngine;
using Grawly.Battle.Modifiers.Important;

namespace Grawly.Battle.Functions {

	/// <summary>
	/// Adds a defense modifier to the combatant so they take half damage, will not get downed, and will not be affected by an ailment.
	/// </summary>
	public class DefendBattleBehaviorFunction : BattleBehaviorFunction {
		...
		/// </summary>
		private static string descriptionText = "Adds a defense modifier to the combatant so they take half damage, will not get downed, and will not be affected by an ailment.";
		/// <summary>
		...
		private IEnumerator ExecuteAsCoroutine(Combatant source, List<Combatant> targets, BattleBehavior battleBehavior) {
			// Debug.LogWarning("Defending. Actually it doesn't do anything yet.");
			source.AddModifier(modifier: new DefendModifier().AssignCombatant(combatant: source));
