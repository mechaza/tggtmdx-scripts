...
namespace Grawly.Editors {
	...
	/// <summary>
	/// An editor window to help with editing battle templates specifically.
	/// </summary>
	public class BattleTemplateEditorWindow : OdinMenuEditorWindow {
		
		#region MAIN CALLS
		// [MenuItem("Grawly/Battle Template Editor")]
		private static void OpenWindow() {
			var window = GetWindow<BattleTemplateEditorWindow>();
			window.position = GUIHelper.GetEditorWindowRect().AlignCenter(800, 600);
			window.titleContent = new GUIContent("Battle Template Editor");
		}
		#endregion
		...
		protected override OdinMenuTree BuildMenuTree() {
			...
			foreach (GamePresetTemplate presetTemplate in allPresetTemplates) {
				...
				// I only want to build out trees for preset templates that have dedicated folders for their battle templates.
				if (presetTemplate.HasBattleTemplatesFolder == false) {
					continue;
				}
