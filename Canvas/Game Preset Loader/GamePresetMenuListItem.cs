...
namespace Grawly.UI {
	public class GamePresetMenuListItem : MenuItem {
		...
		public override void BuildMenuItem(IMenuable item) {
			...
			this.presetTemplate = (GamePresetTemplate)item;

			// Set the screenshot image.
			this.screenshotPreviewImage.overrideSprite = this.presetTemplate.PreviewScreenshot;
			
			// NOTE THAT THIS IS RIPPED FROM THE SETTINGS. THIS ENTIRE LINE IS JUST GONNA MAKE AN EMPTY ITEM PARAMS
			this.subItemContainer.Build(subItemParams: this.presetTemplate is GTSubItem ? (this.presetTemplate as GTSubItem).CurrentSubItemParams : SubItemParams.EmptyParams);

			// Dehighlight it. Note that this will also dehighlight the sub item a second time. I'm okay with that.
			this.Dehighlight(item: this.presetTemplate);
		...
		protected internal override void Dehighlight(IMenuable item) {
			...
			this.subItemContainer.Dehighlight(subItemParams: this.presetTemplate is GTSubItem ? (this.presetTemplate as GTSubItem).CurrentSubItemParams : SubItemParams.EmptyParams);
		...
		protected internal override void Highlight(IMenuable item) {
			...
			// Set the color on the backing image.
			this.backingImage.color = GrawlyColors.colorDict[this.presetTemplate.ColorType];
			this.backingDropshadowImage.color = Color.black;

			// Brighten the screenshot.
			this.screenshotPreviewImage.color = Color.white;
			
			this.subItemContainer.Highlight(subItemParams: this.presetTemplate is GTSubItem ? (this.presetTemplate as GTSubItem).CurrentSubItemParams : SubItemParams.EmptyParams);
		}
		#endregion
